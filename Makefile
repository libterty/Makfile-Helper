# --------------------------------------------------------------------
# Copyright (c) 2021 libtery, The Taiwans. All Rights Reserved.
# Author(s): libtery
# 
...
$(info 'Welcome back $(HOST_USER), you are currently in project of $(PROJECT_NAME) and default branch $(SERVICE_TARGET)')
...
rebuild: ## force to rebuild the image from Dockerfile. In our applications we have a server build on top of Nestjs framework, and Vue Typescript frontend binding with Nginx
				docker-compose -f docker-compose.yaml build --no-cache $(c)
...
sh: ## Is used to see sh into containers. To sh specific container use for example if I only want to sh server container run `make sh c=movies-server`
				docker exec -it $(c) sh
stopall: ## Is used to stop all running containers on your machine
				docker stop $(docker ps -aq)
removeall: ## Is used to reomve all running containers on your machine
